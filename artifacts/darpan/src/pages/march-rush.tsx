...
import { Flame, Clock, ShieldAlert, Award, Calendar, Bell, HelpCircle, CheckCircle, Mail, Phone, Lock } from "lucide-react";
import { formatIndianCurrency } from "@/lib/utils";
import { motion, AnimatePresence } from "framer-motion";

interface MarchRushItem {
  department: string;
  q4_concentration_pct: number;
  risk_tier: "critical" | "high" | "medium" | "low";
  predicted_fraud_types: string[];
  historical_q4_fraud_value: number;
  watch_from_date: string;
}

export default function MarchRush() {
  const [data, setData] = useState<MarchRushItem[]>([]);
  ...
  useEffect(() => {
    fetch("/api/march-rush")
      .then((res) => res.json())
      .then((data) => {
        setData(data.march_rush || []);
        setLoading(false);
      })
      .catch((err) => {
        ...
        >
          ...
          <div className="bg-[#1f1f23]/60 border border-[#2e2e33] rounded-[16px] p-5 flex flex-col items-center gap-3 relative z-10 w-full md:w-auto">
            ...
            <button 
              ...
              className="mt-2 w-full py-2 bg-gradient-to-r from-[#ff385c] to-orange-500 hover:from-rose-600 hover:to-orange-600 text-[12px] font-extrabold text-white rounded-[8px] transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-md"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white border border-[#ebebeb] rounded-[14px] p-5 shadow-sm space-y-2 flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <span className="text-[10px] font-black text-rose-600 bg-rose-50 px-2 py-0.5 rounded-[5px] uppercase tracking-wider">Historical Trend</span>
          ...
          <div className="bg-white border border-[#ebebeb] rounded-[14px] p-5 shadow-sm space-y-2 flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <span className="text-[10px] font-black text-orange-600 bg-orange-50 px-2 py-0.5 rounded-[5px] uppercase tracking-wider">Compressed Bidding</span>
          ...
          <div className="bg-white border border-[#ebebeb] rounded-[14px] p-5 shadow-sm space-y-2 flex flex-col justify-between">
            <div className="flex justify-between items-start">
              <span className="text-[10px] font-black text-[#3f3f3f] bg-[#f7f7f7] px-2 py-0.5 rounded-[5px] uppercase tracking-wider">Corruption Overlay</span>
                      ...
                      return (
                        <tr key={index} className="hover:bg-[#fafafa] transition-colors">
                          ...
                          <td className="py-4 px-4 font-bold text-[#222222]">
                            {item.department}
                            <p className="text-[11px] text-[#aaaaaa] font-medium mt-0.5">Watch active from: {new Date(item.watch_from_date).toLocaleDateString("en-IN", { month: "long", year: "numeric" })}</p>
                          </td>
                          ...
                          <td className="py-4 px-4">
                            <div className="flex flex-wrap gap-1.5">
                              {item.predicted_fraud_types.map((sig, sIdx) => (
                          ...
                          <td className="py-4 px-5 text-right font-extrabold text-[#222222]">
                            {formatIndianCurrency(item.historical_q4_fraud_value)}
                          </td>
                        </tr>
                      );
                    })
              ...
              >
                <div className="flex justify-between items-start border-b border-[#f7f7f7] pb-3">
                  ...
                  <button 
                    ...
                    className="text-[12px] text-[#aaaaaa] hover:text-[#ff385c] font-black"
                  >
                    ✕
                  </button>
                </div>
                ...
                {isSubscribedSuccess ? (
                  ...
                  <form onSubmit={handleSubscribe} className="space-y-4">
                    ...
                    <button 
                      type="submit"
                      className="w-full py-2.5 bg-[#ff385c] hover:bg-[#e0022a] text-white text-[13px] font-extrabold rounded-[8px] transition-all shadow-md cursor-pointer"
                    >
